...
var statusEl = document.getElementById("suiviStatus");
...
async function lookup(ref) {
  ...
  if (!/^KF-[A-Z0-9]+$/.test(ref)) {
    ...
    statusEl.textContent = "Référence invalide. Format attendu : KF-XXXXXXXX.";
    return;
  }
  currentRef = ref;
  try {
    ...
    statusEl.textContent = "Commande trouvée !";
    ...
    statusEl.className = "form-status err";
    statusEl.textContent = "Aucune commande trouvée avec cette référence. Vérifie et réessaie.";
  }
}
